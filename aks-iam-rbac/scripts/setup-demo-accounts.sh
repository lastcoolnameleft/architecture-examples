...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --skip-self-add      Do not add the current Azure CLI user to the InfraOps group"
...
# =============================================================================
# Add deploying admin to InfraOps group (prevents cluster lockout after az aks update)
# =============================================================================
if [ "$SKIP_SELF_ADD" = false ]; then
    SELF_OID=$(az ad signed-in-user show --query id -o tsv 2>/dev/null)
    INFRA_OPS_GROUP_ID="${GROUP_IDS[InfraOps-L2-Elevated]}"
    if [ -n "$SELF_OID" ] && [ -n "$INFRA_OPS_GROUP_ID" ]; then
        echo -e "${BLUE}Adding current Azure CLI identity to InfraOps-L2-Elevated group...${NC}"
        echo -e "${YELLOW}(This prevents cluster lockout after 'az aks update' sets adminGroupObjectIDs)${NC}"
        is_member=$(az ad group member check --group "$INFRA_OPS_GROUP_ID" --member-id "$SELF_OID" --query value -o tsv 2>/dev/null || echo "false")
        if [ "$is_member" = "true" ]; then
            echo -e "  ${YELLOW}Already a member — skipping${NC}"
        else
            az ad group member add --group "$INFRA_OPS_GROUP_ID" --member-id "$SELF_OID" 2>/dev/null && \
                echo -e "  ${GREEN}Added (OID: $SELF_OID)${NC}" || \
                echo -e "  ${RED}Failed — add manually: az ad group member add --group $INFRA_OPS_GROUP_ID --member-id $SELF_OID${NC}"
        fi
        echo -e "  ${YELLOW}NOTE: Log out and back in (az logout && az login) after 'az aks update'${NC}"
        echo -e "  ${YELLOW}to get a fresh token with the new group claim before running kubectl.${NC}"
    fi
else
    echo -e "${YELLOW}Skipping self-add (--skip-self-add). Ensure you are in an admin group before running kubectl.${NC}"
fi
...
INFRA_OPS_L2_USER="demo-infraops@${DOMAIN}"
PLATFORM_SRE_L3_USER="demo-platformsre@${DOMAIN}"
APP_SUPPORT_L2_USER="demo-appsupport@${DOMAIN}"
COMMAND_CENTRE_L1_USER="demo-commandcentre@${DOMAIN}"
CLOUD_DEPLOYMENT_USER="demo-clouddeployment@${DOMAIN}"
VIEWER_USER="demo-viewer@${DOMAIN}"
...
# Write group-ids.env into each kustomize directory (kustomize security policy prevents ../)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
RBAC_DIR="${SCRIPT_DIR}/../rbac"
for KUSTOMIZE_DIR in cluster namespace; do
    cat > "${RBAC_DIR}/${KUSTOMIZE_DIR}/group-ids.env" << EOF
# Generated by setup-demo-accounts.sh on $(date) — do not edit manually
INFRA_OPS_L2_GROUP_ID=${GROUP_IDS[InfraOps-L2-Elevated]}
PLATFORM_SRE_L3_GROUP_ID=${GROUP_IDS[PlatformSRE-L3-Elevated]}
APP_SUPPORT_L2_GROUP_ID=${GROUP_IDS[AppSupport-L2-Elevated]}
COMMAND_CENTRE_L1_GROUP_ID=${GROUP_IDS[CommandCentre-L1-Elevated]}
...
echo -e "${GREEN}Kustomize group IDs saved to: rbac/cluster/group-ids.env and rbac/namespace/group-ids.env${NC}"
echo ""
